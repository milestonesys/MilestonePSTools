BeforeAll {
    ...
    $excludes = '.devcontainer', '.cache', '.vscode', 'Output'
    $allTextFiles      = Get-ChildItem $projectRoot | Where-Object Name -notin $excludes | Get-ChildItem -File -Recurse | Where-Object Extension -in $txtFileExtentions
    $unicodeFilesCount = 0
    $totalTabsCount    = 0
