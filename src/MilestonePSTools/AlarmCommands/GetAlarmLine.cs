...
{
    /// <summary>
    /// <para type="synopsis">Gets Alarms from the Event Server</para>
    /// <para type="description">Gets a list of Alarms Event Server using the AlarmCommandClient / IAlarmCommand interface.</para>
    /// <example>
    ///     <code>
    /// PS C:\> $c1 = New-AlarmCondition -Operator GreaterThan -Target Timestamp -Value (Get-Date).Date.AddDays(-1).ToUniversalTime()
    ...
    /// PS C:\> Get-AlarmLine -Conditions $c1,$c2 -SortOrders $order | Select LocalId, Timestamp, Message, SourceName, State</code>
    ///     <para>Create Conditions to filter the AlarmLines to only those alarms with a timestamp occurring 'Yesterday',
    ///     and order the results in descending order by time.</para>
    ///     <para/><para/><para/>
    /// </example>
    /// </summary>
    [Cmdlet(VerbsCommon.Get, nameof(AlarmLine), DefaultParameterSetName = "GetAlarmLines")]
    [OutputType(typeof(AlarmLine))]
    [RequiresVmsConnection()]
    public class GetAlarmLine : ConfigApiCmdlet, IDisposable
    {
        ...
        /// </summary>
        [Parameter(ParameterSetName = "GetAlarmLines")]
        ...
        protected override void ProcessRecord()
        {
            ...
            else
            {
                ...
                var index = StartAt;
                AlarmLine[] alarmLines;
                do
                {
                    alarmLines = _alarmClient.GetAlarmLines(index, PageSize, filter);
                    foreach (var alarmLine in alarmLines)
                    {
                        WriteObject(alarmLine);
                    }

                    index += PageSize;
                } while (!SinglePage && alarmLines.Length == PageSize);
            }
        }
