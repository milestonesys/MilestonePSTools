...
The installation scripts provided [here](../../getting-started.md#installation) are designed to protect you from common errors when you install modules from PowerShell Gallery or run commands or scripts for the first time. Following are a collection of these errors so when you run into them, you'll know what to do!
...
After I run this on a clean Windows 10 Sandbox instance, the output of `#!powershell [Net.ServicePointManager]::SecurityProtocol` is `Tls11, Tls12`, indicating that the supported protocols include TLS 1.1 and TLS 1.2.
