...
## Windows PowerShell ISE
 ...
 You can also run one or more lines of code at a time using ++f8++ or run the whole file using ++f5++. When you get comfortable with PowerShell as a language, and the ISE environment, you can even add break points and *debug* your scripts when they do the unexpected.
...
These are the 32-bit equivalents of the same two PowerShell environments already mentioned. The standard PowerShell environments are 64-bit and you will rarely need a 32-bit environment. But if you need it, you have it. Since the Milestone MIP SDK is provided through 64-bit NuGet packages and the 32-bit MIP SDK is deprecated, you will require a 64-bit Windows PowerShell 5.1 environment to use the MilestonePSTools PowerShell module.
