...
online version: https://www.milestonepstools.com/commands/en-US/Set-VmsSpeaker/
schema: 2.0.0
---
...
```
Set-VmsSpeaker [[-RecordingEnabled] <Boolean>] [[-ManualRecordingTimeoutEnabled] <Boolean>]
...
Get-VmsHardware | Get-VmsSpeaker -Channel (1..63) -Verbose | Set-VmsSpeaker -Enabled $false
```

In this example, we first ensure we are logged in to the Management Server. Then
we gets all enabled speakers with a channel number between 1 and 63 (speaker 2 to
speaker 64), and disable them. You might do this if you only use the first channel
on every speaker, and all other channels should be disabled. In this case, channels
higher than 63 would be unchanged. If you had devices with more unused channels
than this, you could use "(1..511)" to select all speakers from the 2nd to the
512th channel.

### Example 2
```powershell
$splat = @{
    InputObject       = Get-VmsSpeaker
    PrebufferEnabled  = $true
    PrebufferSeconds  = 10
    PrebufferInMemory = $true
}
Set-VmsSpeaker @splat
```

Configure an in-memory prebuffer on all enabled speakers with a duration of 10 seconds.

### Example 3
```powershell
$splat = @{
    InputObject = Get-VmsSpeaker -Name 'Office Entrance'
