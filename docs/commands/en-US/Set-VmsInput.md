...
online version: https://www.milestonepstools.com/commands/en-US/Set-VmsInput/
schema: 2.0.0
---
...
```
Set-VmsInput [-InputObject] <IConfigurationItem[]> [[-Name] <String>] [[-ShortName] <String>]
...
Get-VmsHardware | Get-VmsInput -Channel (1..63) -Verbose | Set-VmsInput -Enabled $false -Verbose
```

In this example, we first ensure we are logged in to the Management Server. Then
we gets all enabled inputs with a channel number between 1 and 63 (input 2 to
input 64), and disable them. You might do this if you only use the first channel
on every input, and all other channels should be disabled. In this case, channels
higher than 63 would be unchanged. If you had devices with more unused channels
than this, you could use "(1..511)" to select all inputs from the 2nd to the
512th channel.

### Example 2
```powershell
$splat = @{
    InputObject = Get-VmsInput -Name 'Office Entrance'
...
### -InputObject
Specifies one or more `InputEvent` objects as returned by `Get-VmsInput`.
