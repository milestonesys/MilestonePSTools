...
online version: https://www.milestonepstools.com/commands/en-US/Get-VmsSpeaker/
schema: 2.0.0
---
...
## SYNOPSIS
Gets speaker devices from the currently connected XProtect VMS site.

## SYNTAX

### QueryItems (Default)
```
Get-VmsSpeaker [[-Name] <String>] [[-Description] <String>] [[-Channel] <Int32[]>]
 [[-EnableFilter] <EnableFilter>] [[-Comparison] <Operator>] [[-MaxResults] <Int32>] [<CommonParameters>]
```

### Hardware
```
Get-VmsSpeaker [-Hardware] <Hardware[]> [[-Name] <String>] [[-Description] <String>] [[-Channel] <Int32[]>]
 [[-EnableFilter] <EnableFilter>] [[-Comparison] <Operator>] [<CommonParameters>]
```

### Id
```
Get-VmsSpeaker [-Id] <Guid[]> [<CommonParameters>]
```

### Path
```
Get-VmsSpeaker [-Path] <String[]> [<CommonParameters>]
```

## DESCRIPTION
The `Get-VmsSpeaker` cmdlet gets speaker devices from the currently connected XProtect VMS site. Speakers are logical child
devices attached to "hardware" which are most often an IP camera. The `VideoOS.Platform.ConfigurationItems.Speaker`
object returned from this command represents a configurable device on the XProtect VMS.

The corresponding `Set-VmsSpeaker` command can be used in conjunction with this cmdlet to modify the device settings.
...
Get all enabled speaker devices.

### Example 2
```powershell
$recorder = Get-VmsRecordingServer | Out-GridView -OutputMode Multiple
$recorder | Get-VmsHardware | Get-VmsSpeaker
```

Get all enabled speaker devices from the selected recording server(s).

### Example 3
```powershell
Get-VmsSpeaker -Name 'garage'
```

Get all enabled speakers with the case-insensitive word "garage" in the name.

### Example 4
```powershell
Get-VmsSpeaker -EnableFilter Disabled
```

Get all **disabled** speakers.

### Example 5
```powershell
Get-VmsSpeaker -EnableFilter All -Channel 0 | Set-VmsSpeaker -Enabled $true -PassThru
Get-VmsSpeaker -EnableFilter All -Channel (1..64) | Set-VmsSpeaker -Enabled $false -PassThru
```

Get all speakers with a channel value of "0" (port number 1), and enable them if they aren't enabled already. Then get all
speakers with a higher channel number from 1 to 64, and disable them. In most cases this should leave you with only the
first speaker enabled.
