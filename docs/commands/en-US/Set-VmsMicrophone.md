...
online version: https://www.milestonepstools.com/commands/en-US/Set-VmsMicrophone/
schema: 2.0.0
---
...
```
Set-VmsMicrophone [[-RecordingEnabled] <Boolean>] [[-ManualRecordingTimeoutEnabled] <Boolean>]
...
Get-VmsHardware | Get-VmsMicrophone -Channel (1..63) -Verbose | Set-VmsMicrophone -Enabled $false
```

In this example, we first ensure we are logged in to the Management Server. Then
we gets all enabled microphones with a channel number between 1 and 63 (microphone 2 to
microphone 64), and disable them. You might do this if you only use the first channel
on every microphone, and all other channels should be disabled. In this case, channels
higher than 63 would be unchanged. If you had devices with more unused channels
than this, you could use "(1..511)" to select all microphones from the 2nd to the
512th channel.

### Example 2
```powershell
$splat = @{
    InputObject       = Get-VmsMicrophone
    PrebufferEnabled  = $true
    PrebufferSeconds  = 10
    PrebufferInMemory = $true
}
Set-VmsMicrophone @splat
```

Configure an in-memory prebuffer on all enabled microphones with a duration of 10 seconds.

### Example 3
```powershell
$splat = @{
    InputObject = Get-VmsMicrophone -Name 'Office Entrance'
...
longitude are positive or negative numbers. For example, "45.4171601197572, -122.732137977298".

To remove the coordinates from a device, you may set the value to `$null` or an empty string.
