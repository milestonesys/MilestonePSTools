...
online version: https://www.milestonepstools.com/commands/en-US/Get-VmsMetadata/
schema: 2.0.0
---
...
## SYNOPSIS
Gets metadata devices from the currently connected XProtect VMS site.

## SYNTAX

### QueryItems (Default)
```
Get-VmsMetadata [[-Name] <String>] [[-Description] <String>] [[-Channel] <Int32[]>]
 [[-EnableFilter] <EnableFilter>] [[-Comparison] <Operator>] [[-MaxResults] <Int32>] [<CommonParameters>]
```

### Hardware
```
Get-VmsMetadata [-Hardware] <Hardware[]> [[-Name] <String>] [[-Description] <String>] [[-Channel] <Int32[]>]
 [[-EnableFilter] <EnableFilter>] [[-Comparison] <Operator>] [<CommonParameters>]
```

### Id
```
Get-VmsMetadata [-Id] <Guid[]> [<CommonParameters>]
```

### Path
```
Get-VmsMetadata [-Path] <String[]> [<CommonParameters>]
```

## DESCRIPTION
The `Get-VmsMetadata` cmdlet gets metadata devices from the currently connected XProtect VMS site. Metadatas are
logical child devices attached to "hardware" which are most often an IP camera. The `VideoOS.Platform.ConfigurationItems.Metadata`
object returned from this command represents a configurable device on the XProtect VMS.

The corresponding `Set-VmsMetadata` command can be used in conjunction with this cmdlet to modify the device settings.
...
Get all enabled metadata devices.

### Example 2
```powershell
$recorder = Get-VmsRecordingServer | Out-GridView -OutputMode Multiple
$recorder | Get-VmsHardware | Get-VmsMetadata
```

Get all enabled metadata devices from the selected recording server(s).

### Example 3
```powershell
Get-VmsMetadata -Name 'garage'
```

Get all enabled metadatas with the case-insensitive word "garage" in the name.

### Example 4
```powershell
Get-VmsMetadata -EnableFilter Disabled
```

Get all **disabled** metadatas.

### Example 5
```powershell
Get-VmsMetadata -EnableFilter All -Channel 0 | Set-VmsMetadata -Enabled $true -PassThru
Get-VmsMetadata -EnableFilter All -Channel (1..64) | Set-VmsMetadata -Enabled $false -PassThru
```

Get all metadatas with a channel value of "0" (port number 1), and enable them if they aren't enabled already. Then get all
metadatas with a higher channel number from 1 to 64, and disable them. In most cases this should leave you with only the
first metadata enabled.
