...
online version: https://www.milestonepstools.com/commands/en-US/Get-VmsMetadataLiveRecord/
schema: 2.0.0
---
...
## SYNOPSIS
Gets one or more records from a live metadata device stream.

## SYNTAX

### Metadata (Default)
```
Get-VmsMetadataLiveRecord [-Until <DateTime>] [-Count <Int32>] -Metadata <Metadata> [-Raw] [<CommonParameters>]
```

### Camera
```
Get-VmsMetadataLiveRecord [-Until <DateTime>] [-Count <Int32>] -Camera <Camera> [-Raw] [<CommonParameters>]
```

### Id
```
Get-VmsMetadataLiveRecord [-Until <DateTime>] [-Count <Int32>] -Id <Guid> [-Raw] [<CommonParameters>]
```

## DESCRIPTION
The `Get-VmsMetadataLiveRecord` cmdlet gets one or more records from a live metadata device stream. Metadata records are
usually XML documents based on the [ONVIF Metadata Stream schema](https://github.com/onvif/specs/blob/development/wsdl/ver10/schema/metadatastream.xsd).
The actual format for metadata records depends on the device driver used in XProtect.

Metadata records can be retrieved until the value of the `Until` parameter if provided, or a number of records can be
returned based on the `Count` parameters.
...
$camera | Get-VmsMetadataLiveRecord -Until (Get-Date).AddHours(1) | Where-Object {
    'Vehical' -in $_.GetMetadataStream().VideoAnalyticsItems.Frames.Objects.Appearance.Class.ClassCandidates.Type
} | ForEach-Object {
...
This example retrieves live metadata records for an hour from a user-selected camera with at least one vehicle, and
...
### -Count
Specifies the number of live metadata records to return. This parameter is ignored when using the `Until` parameter.
...
### -Until
When specified, all metadata records received will be returned until this time.
