...
online version: https://www.milestonepstools.com/commands/en-US/Set-VmsDevice/
schema: 2.0.0
---
...
```
Set-VmsDevice [-InputObject] <IConfigurationItem[]> [[-Name] <String>] [[-ShortName] <String>]
...
Get-VmsHardware | Get-VmsDevice -Channel (1..63) -Verbose | Set-VmsDevice -Enabled $false -Verbose
```

In this example, we first ensure we are logged in to the Management Server. Then
we gets all enabled devices with a channel number between 1 and 63 (channel 2 to
channel 64), and disable them. You might do this if you only use the first channel
on every device, and all other channels should be disabled. In this case, channels
higher than 63 would be unchanged. If you had devices with more unused channels
than this, you could use "(1..511)" to select all devices from the 2nd to the
512th channel.

### Example 2
```powershell
$splat = @{
    InputObject = Get-VmsDevice -Name 'Office Entrance'
...
### -InputObject
Specifies one or more Hardware child devices.
